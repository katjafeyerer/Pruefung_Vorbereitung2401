...
public class InOrderTest {
	...
	@Test
	public void testGetNodesInOrder11() {
		List<String> ls = ts.getNodesInOrder2(ts.find(new Film("Avanti!", 2.33)));
		assertEquals(1, ls.size());
		assertEquals("Avanti!", ls.get(0));
	}

	@Test
	public void testGetNodesInOrder21() {
		List<String> ls = ts.getNodesInOrder2(ts.find(new Film("Alien", 1.95)));
		assertEquals(3, ls.size());
		assertEquals("Adaption", ls.get(0));
		assertEquals("Alien", ls.get(1));
		assertEquals("The Great Dictator", ls.get(2));
	...
	@Test
	public void testGetNodesInOrder31() {
		List<String> ls = ts.getNodesInOrder2(ts.getRoot());
		assertEquals(10, ls.size());
		assertEquals("Office Space", ls.get(0));
		assertEquals("Groundhog Day", ls.get(1));
		assertEquals("Ferris Bueller's Day Off", ls.get(2));
